    ...
    <script>
        ...
        function startSession(dI) {{
            ...
            d.exercises.forEach((ex, eI) => {{
                ...
                const tecnicaText = ex.tecnica || ex.notes || "";
                const isCircuit = tecnicaText.includes('+') || ex.name.toLowerCase().includes('circuito');
                let labelHtml = `<div style="flex:1"><b>${{sanitize(ex.name)}}</b>`;
                if (tecnicaText && !isCircuit) {{
                    labelHtml += `<br><small style="font-size:0.65rem; opacity:0.6; font-family:'JetBrains Mono'; color:var(--accent-pink);">// TEC: ${{tecnicaText}}</small>`;
                }}
                labelHtml += `</div><div style="font-size:0.65rem; opacity:0.6; font-family:'JetBrains Mono'">${{ex.sets}}S</div>`;
        ...
        function openExercise(eI) {{
            ...
            const tecnicaText = ex.tecnica || ex.notes || "";
            let subExs = [];
            if (tecnicaText && (tecnicaText.includes('+') || ex.name.toLowerCase().includes('circuito'))) {{
                subExs = tecnicaText.split('+').map(s => {{
            ...
            document.getElementById('ex-header').innerHTML = `
                <div class="tech-card" style="transform:none;">
                    ...
                    <div style="margin-top:15px; border-top:1.5px dashed rgba(0,0,0,0.15); padding-top:15px;">
                        ...
                        <div style="font-family:'JetBrains Mono', monospace; font-size:0.8rem; font-weight:700; color:#000;">
                            ${{tecnicaText}}
                        </div>
                    </div>
                    ` : ''}}
