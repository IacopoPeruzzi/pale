...
<head>
    ...
    <style>
        ...
        * {{ box-sizing: border-box; -webkit-tap-highlight-color: transparent; outline: none; }}
        ...
        .load-input-wrap {{ flex: 1; display: flex; align-items: center; gap: 8px; }}
        .load-input {{ 
...
<body>
    ...
    <div id="view-exercise" class="view">
        ...
        <div id="sets-rows-container"></div>
        ...
        ${{parsed_workout and parsed_workout.get('notes') ? f'<div class="note-box" style="margin-top:20px">{{parsed_workout["notes"]}}</div>' : ''}}

        <div class="nav-dock">
            <button class="btn btn-alt" onclick="startTimer()" style="flex: 0.4">TIMER</button>
    ...
    <script>
        ...
        function toggleSet(sI) {{
            ...
            data.sets[sI].done = !data.sets[sI].done;
            if(data.sets[sI].done) startTimer();
        ...
        function updateExMeta() {{
            ...
            document.getElementById('ex-detail-meta').innerText = `Serie ${{done}} di ${{data.sets.length}} completate`;
            data.completed = (done === data.sets.length);
            save();
        }}
        ...
        function exportToCSV() {{
            ...
            let csv = "Data,Piano,Settimana,Giorno,Esercizio,Serie,Carico,Note\\n";
            ...
            d.exercises.forEach(ex => {{
                ...
                if(data) {{
                    data.sets.forEach((s, i) => {{
                        csv += `"${{date}}","${{w.title}}","W${{currentWeek}}","${{d.name}}","${{ex.name}}",${{i+1}},"${{s.load}}","${{data.notes.replace(/"/g, '""')}}"\\n`;
                    }});
                }}
            }});
...
if __name__ == "__main__":
    ...
    print(f"Successo: Tracking carichi e note sessione implementato con export CSV.")
