    ...
    <script>
        ...
        function exportToCSV() {{
            ...
            d.exercises.forEach(ex => {{
                ...
                if(data) {{
                    data.sets.forEach((s, i) => {{
                        csv += `"${{date}}","${{w.title}}","W${{currentWeek}}","${{sanitize(d.name)}}","${{sanitize(ex.name)}}",${{i+1}},"${{s.load}}","${{data.notes.replace(/"/g, '""')}}"\\n`;
                    }});
                }}
            }});
