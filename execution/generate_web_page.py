...
<body>
    ...
    <script>
        ...
        function sanitizeName(name) {{
            return name.replace(/^#+\\s*/, '').replace(/^\\*+\\s*/, '').replace(/^-\\s*/, '').trim();
        }}
        ...
        function openWorkout(idx) {{
            ...
            document.getElementById('plan-title').innerText = sanitizeName(w.title);
            updateProg(w);
            ...
            w.days.forEach((d, dI) => {{
                ...
                const cleanName = sanitizeName(d.name.split('-')[1] || d.name);
                c.innerHTML = `
                    <div class="drag-handle">☰</div>
                    <div class="day-content">
                        <h4 class="day-num-title">GIORNO ${{dI + 1}}</h4>
                        <small>${{cleanName}}</small>
                    </div>
        ...
        function startSession(dI) {{
            ...
            const d = w.days[dI];
            const status = w.progress && w.progress[currentWeek] && w.progress[currentWeek][dI];
            const cleanDayName = sanitizeName(d.name.split('-')[1] || d.name);
            
            document.getElementById('session-title').innerText = `GIORNO ${{dI + 1}}`;
            document.getElementById('session-subtitle').innerText = `WEEK ${{currentWeek}} • ${{cleanDayName}}`;
                ...
                card.innerHTML = `
                    ...
                    <div class="ex-info">
                        <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:10px;">
                            <div class="ex-name">${{sanitizeName(ex.name)}}</div>
                        ...
                        ${{ex.notes ? `<div class="note-box">${{ex.notes}}</div>` : ''}}
                    </div>
                `;
...
if __name__ == "__main__":
    ...
    print(f"Successo: Pulizia markdown e miglioramento visualizzazione esercizi.")
